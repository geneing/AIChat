package com.eugene.aichat.core.ai.request
...
class ContextAssemblerTest {
    ...
    fun `tools are forwarded only when model supports them`() = runTest {
        ...
        val tools = listOf(
            com.eugene.aichat.core.network.dto.ToolDefinitionDto(
                function = com.eugene.aichat.core.network.dto.ToolFunctionDto(
                    name = "demo",
                    description = "d",
...
private class FakeSkillRepo(initial: List<SkillEntity>) : SkillRepository {
    ...
    override fun observeAll(): kotlinx.coroutines.flow.Flow<List<com.eugene.aichat.core.domain.model.Skill>> =
        MutableStateFlow(store.values.map { it.toDomain() })
    override fun observeEnabled(): kotlinx.coroutines.flow.Flow<List<com.eugene.aichat.core.domain.model.Skill>> =
    ...
    override suspend fun upsert(skill: com.eugene.aichat.core.domain.model.Skill) = Unit
...
private fun SkillEntity.toDomain(): com.eugene.aichat.core.domain.model.Skill =
    com.eugene.aichat.core.domain.model.Skill(
...
private class FakeAgentRepo(initial: List<AgentEntity>) : AgentRepository {
    private val store = initial.associateBy { it.id }.toMutableMap()
    override fun observeAll(): kotlinx.coroutines.flow.Flow<List<com.eugene.aichat.core.domain.model.Agent>> =
        MutableStateFlow(store.values.map { it.toDomain() })
    override fun observeEnabled(): kotlinx.coroutines.flow.Flow<List<com.eugene.aichat.core.domain.model.Agent>> =
        MutableStateFlow(store.values.filter { it.isEnabled }.map { it.toDomain() })
    override suspend fun getById(id: String) = store[id]?.toDomain()
    override suspend fun upsert(agent: com.eugene.aichat.core.domain.model.Agent) = Unit
    override suspend fun setEnabled(id: String, enabled: Boolean) = Unit
    override suspend fun deleteUserAgent(id: String) = Unit
    override suspend fun ensureSeeded() = Unit
}

private fun AgentEntity.toDomain(): com.eugene.aichat.core.domain.model.Agent =
    com.eugene.aichat.core.domain.model.Agent(
        id = id, name = name, description = description,
